...
def scan(code: String, outDir: String) = {
    println("Processing: " + code)

    val outFileName = Paths.get(code).getFileName + ".json"
    val outFilePath = Paths.get(outDir).resolve(outFileName)


    if (Files.exists(outFilePath)) {
        println("Found previous result... skipping scan")
    } else if (!code.endsWith(".jar")) {
        println("NOT JAR file... skipping scan")
    } else {
        println("Scanning: " + code)
        println("Result path: " + outFilePath.toString())
        importCode(code)
        ...
        //browse(result.l)
        result.toJsonPretty |> outFilePath.toString()
    }
}


@main def main(inDir: String, outDir: String) = {
    ...
    Files.list(FileSystems.getDefault.getPath(inDir)).iterator().asScala.foreach(
        f => scan(f.toString(), outDir)
    )
}

// joern --script all_sinks.sc --params inDir=jars,outDir=result
